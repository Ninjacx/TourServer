...
router.get('/homeData',async (req, res, next)=>{
	...
	var contentsql = `select count(t_support.id) as supportCount,t_plate.plate_name,t_plate_second.plate_name as secondPlate_name,t_content.*,
						...
						left join t_user on t_content.user_id = t_user.id where t_content.is_del = 0 ${type} group by t_content.id order by t_content.create_time desc limit 10 offset ${offSets}`;
				...
				//  关联图片是属于哪条内容的ID
					for (let j = 0; j < TextArr.length; j++) {
						if(ImgTextId[index] == j){
							...
							insertStr+=`("http://qbc93xu4f.bkt.clouddn.com/${fileName}${imagePath}","${TextArr[j].txtImgId}","${contentId}"),`;
						}
					}
					// 上传到七牛云
