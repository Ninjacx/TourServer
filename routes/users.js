...
router.post('/login', function(req, res, next) {
        ...
        if(state==1){
          ...
          sql = `select t_user.nick_name,t_user.token,t_user.icon,t_user.icon,t_user.member_id,t_user.signature,t_user.is_freeze from t_icode left join t_user on t_icode.phone = t_user.phone where t_icode.phone = "${phone}" and  t_icode.code = "${code}"`;
          msg = "验证码错误";
        }else if(state==2){
